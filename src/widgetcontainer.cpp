...
QWidget *WidgetContainer::createContainer(const QString &iAppName,
                                          bool iAutoOpen) {
  DWORD procId = (DWORD)findProcessId(iAppName);

  qDebug() << "procId:" << procId;
  ...
  EnumWindows(
      [](HWND wnd, LPARAM lParam) -> BOOL {
        ...
        BOOL isMainWnd =
            GetWindow(wnd, GW_OWNER) == nullptr && IsWindowVisible(wnd);
...
DWORD WidgetContainer::findProcessId(const QString &iAppName) {
  ...
  do {
    ...
    //        pe32.th32ProcessID); if (hProcess == nullptr) {
    //            continue;
    //        }
    if (std::wstring(pe32.szExeFile).find(iAppName.toStdWString()) !=
        std::wstring::npos) {
...
DWORD WidgetContainer::startUpProcess(const QString &iAppName) {
  ...
  CreateProcess(filePath.c_str(), // the path
                ...
                &pi // Pointer to PROCESS_INFORMATION structure (removed extra
                    // parentheses)
  );
...
HANDLE WidgetContainer::findWindowHandle(const QString &iClassName,
                                         const QString &iWindowName) {
  // L"Notepad++", L"new 1 - Notepad++ [Administrator]"
#ifdef UNICODE
  return FindWindow(iClassName.toStdWString().data(),
                    iWindowName.toStdWString().data());
#else
  return FindWindow(iClassName.toStdString().data(),
                    iWindowName.toStdString().data());
#endif
}
